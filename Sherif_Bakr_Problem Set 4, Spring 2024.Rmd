---
title: "Problem Set 4, Spring 2024"
author: "Sherif Bakr"
output:
...
fish$fisherman_factor <- factor(fish$fisherman, levels=c(0, 1), labels=c("No", "Yes"))
fish$fishpart_factor <- factor(fish$fishpart, levels=c(0, 1, 2, 3), labels=c("None", "Muscle Tissue Only", "Muscle and Whole Fish", "Whole Fish"))
...
null_model <- lm(logTotHg ~ 1, data = fish.auto)
summary(null_model)
full_formula <- as.formula("logTotHg ~ fisherman_factor + age + restime + height + weight + fishmlwk + fishpart_factor")
forward_model <- step(null_model, scope = full_formula, direction = "forward", trace = 1)
...
full_model <- lm(logTotHg ~ fisherman_factor + age + restime + height + weight + fishmlwk + fishpart_factor, data = fish.auto)
summary(full_model)
null_formula <- as.formula("logTotHg ~ 1")
null_formula
backward_model <- step(full_model, scope = null_formula, direction = "backward", trace = 1)
...
# 1) Code for conducting a best subsets selection

library(leaps)
fmla.full <- as.formula("logTotHg ~ fisherman_factor + fishpart_factor + age + restime + height + weight + fishmlwk")
x <- model.matrix(fmla.full, data=fish.auto) ##
y <- log(fish$TotHg)
#print(y)
x
...
minBIC <- min(BICs)
best.subset.bic <- which(BICs == minBIC)[1]
best.subset.bic
```
...
# 2) Display the model selected by best subsets selection 

varnames <- attr(subsets, "dimnames")[[2]]
varnames.best <- varnames[subsets[best.subset.bic,]]
bestsubsets.model <- lm(log(TotHg) ~ weight + fishpart_factor, data=fish)
...
n <- nrow(fish.auto)  
BIC_values <- summary(bestsubsets.model)$bic
k_values <- seq(1, 7)  
AIC_values <- BIC_values + 2 * k_values - 2 * log(n)
...
selected_model_aic <- summary(bestsubsets.model)$which[best_model_aic, ]
selected_predictors_aic <- names(selected_model_aic)[-1]
selected_formula_aic <- logTotHg ~ fisherman_factor + age + restime + height + weight + fishmlwk + fishpart_factor
final_model_aic <- lm(selected_formula_aic, data = fish.auto)
######
...
Your answer here: The predictors included in the model chosen using forward selection are weight and fishpart_factor (Muscle Tissue Only, Muscle and Whole Fish, Whole Fish).

Question B: Which predictors were included in the model you chose using backward selection?

Your answer here: The predictors included in the model chosen using backward selection are weight and fishpart_factor (Muscle Tissue Only, Muscle and Whole Fish, Whole Fish).

Question C: Which predictors were included in the model you chose using best subsets selection (AIC)?

Your answer here: fisherman_factor, age, restime, height, weight, fishmlwk, and fishpart_factor (Muscle Tissue Only, Muscle and Whole Fish, Whole Fish).

Question D: Which predictors were included in the model you chose using best subsets selection (BIC)?

Your answer here: Weight, and fishpart_factor (Muscle Tissue Only, Muscle and Whole Fish, Whole Fish).
