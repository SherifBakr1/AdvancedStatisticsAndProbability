---
title: "Problem Set 6, Spring 2024"
...
Your answer here: Yes, the number of 1s in the worse variable (1900) matches the number of people who responded "worse today".
...
pew.life2 <- pew.life %>%
  select(worse, income, gender, eth, ideo, edu)
str(pew.life2)
...
Your answer here: Model 3 should be chosen as it has the lowest validation deviance of 799.8163.
...
x <- model.matrix(medv ~ ., data = BostonHousing)[, -1] # Remove the intercept column
...
cvfit.house.ridge <- cv.glmnet(x, y, alpha = 0, standardize = TRUE)
...
coef_lambda_min <- coef(cvfit.house.ridge, s = lambda_min)
coef_lambda_min
...
lambda_1se <- cvfit.house.ridge$lambda.1se
lambda_1se

# Display the coefficients associated with lambda.1se
coef_lambda_1se <- coef(cvfit.house.ridge, s = lambda_1se)
coef_lambda_1se
...
cvfit.house.lasso <- cv.glmnet(x, y, alpha = 1, standardize = TRUE)
...
lambda_min_lasso <- cvfit.house.lasso$lambda.min
lambda_min_lasso

# Display the coefficients associated with lambda.min
coef_lambda_min_lasso <- coef(cvfit.house.lasso, s = lambda_min_lasso)
coef_lambda_min_lasso
...
lambda_1se_lasso <- cvfit.house.lasso$lambda.1se
lambda_1se_lasso

# Display the coefficients associated with lambda.1se here
coef_lambda_1se_lasso <- coef(cvfit.house.lasso, s = lambda_1se_lasso)
coef_lambda_1se_lasso
...
Your answer here: lambda.1se had more coefficients set to zero compared to lambda.min
